...
const startLogoutTimer = function () {
  ...
  const tick = function () {
    ...
    //ckeck o s and stop timer and log out user
    if (time === 0) {
      clearInterval(timer);
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const display = formatDate(date, acc.locale);
...
const calcDisplaySummary = function (acc) {
  ...
  const outs = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, withrawl) => acc + withrawl, 0);
...
// const first = accounts.filter(acc => acc.owner === 'Jonas Schmedtmann')[0];
// console.log(first);
...
// console.log(Array.from({ length: 7 }, cur => 1));
// console.log(typeof new Set([1, 1, 2]));
